...
{
	union litmus_syscall_args args;
	args.run_add_node.id = id;
	args.run_add_node.rate_a = rate_a;
	args.run_add_node.rate_b = rate_b;
	args.run_add_node.level = level;
	return syscall(LRT_run_add_node, (unsigned long) &args);
}
